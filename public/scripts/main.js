...
if(confirmHostPasswordBtn) confirmHostPasswordBtn.addEventListener('click', () => {
    ...
    if(destroyRoomBtn) destroyRoomBtn.classList.remove('hidden');
...
if (destroyRoomBtn) {
    destroyRoomBtn.addEventListener('click', () => {
        if (confirm('هل أنت متأكد من إغلاق الغرفة وطرد جميع اللاعبين للعودة للقائمة الرئيسية؟')) {
            socket.emit('leaveRoom'); sessionStorage.clear(); window.location.href = window.location.pathname;
        }
    });
}
...
socket.on('updatePlayers', (playersArray) => {
    ...
    if (playersListDiv) { 
        ...
        playersArray.forEach(player => { 
            ...
            const hostBadge = player.isHost ? '<span style="color:#00f3ff; font-weight:bold;">👑 هوست</span>' : ''; 
            const youBadge = isMe ? '<span style="color:#00ff88; margin-right:5px;">(أنت)</span>' : '';
            playersHTML += `
            <div class="player-item">
                <div class="player-avatar">👤</div>
                <div class="player-info">
                    <div class="player-name">${player.name}</div>
                    <div class="player-status">${hostBadge} ${youBadge}</div>
                </div>
            </div>`; 
        }); 
    ...
    if (isHost && modalPlayersList) { 
        let modalHTML = ''; 
        playersArray.forEach(player => { 
            const crown = player.isHost ? '<span style="color:#ff0055;">👑</span>' : ''; 
            const isMe = player.id === myPlayerId; 
            const actionButtons = player.isHost 
                ? `<button class="btn-sidebar edit" style="width:auto; padding:5px 10px;" onclick="editPlayerName('${player.id}')">✏️</button>` 
                : `<button class="btn-sidebar edit" style="width:auto; padding:5px 10px;" onclick="editPlayerName('${player.id}')">✏️</button><button class="btn-sidebar btn-danger" style="width:auto; padding:5px 10px; margin-right:5px;" onclick="kickPlayer('${player.id}')">❌</button>`; 
