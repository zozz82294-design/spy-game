...
const rebusPuzzlesDB = [
    ...
    { category: "مكان", clue: "م + 🍞 + ز", answer: "مخبز", answerEn: ["bakery"] },
    { category: "شيء", clue: "س + 🛏️ + ر", answer: "سرير", answerEn: ["bed"] },
    { category: "رياضة", clue: "م + 🥊 + مة", answer: "ملاكمة", answerEn: ["boxing"] },
    { category: "أداة", clue: "ص + 🚀 + وخ", answer: "صاروخ", answerEn: ["rocket"] },
...
function checkVotingResult(roomId) {
    ...
    if(rooms[roomId].gameMode === 'spy2' && rooms[roomId].kickedPlayers) {
        totalPlayers -= rooms[roomId].kickedPlayers.length;
    }
    ...
    if (rooms[roomId].gameMode === 'spy2' && !isSpyCaught) {
        ...
        setTimeout(() => {
            if (rooms[roomId] && rooms[roomId].gameState === 'spy_decision') {
                io.to(roomId).emit('spyDecisionPhase');
            }
        }, 5000);
...
function handlePlayerLeave(roomId, playerId) {
    ...
    if (rooms[roomId].spyId === playerId && ['playing', 'voting', 'guessing', 'voting_result', 'voting_tied', 'spy_decision'].includes(rooms[roomId].gameState)) { 
        if(rooms[roomId].guessTimer) clearTimeout(rooms[roomId].guessTimer); 
        if(rooms[roomId].tieTimer) clearTimeout(rooms[roomId].tieTimer); 
        rooms[roomId].gameState = 'waiting'; 
        rooms[roomId].votes = {}; 
        rooms[roomId].kickedPlayers = [];
    ...
    if (rooms[roomId]) {
        ...
        if (wasVoting && !gameAborted) {
            ...
            let remainingPlayersCount = Object.keys(rooms[roomId].players).length;
            if(rooms[roomId].gameMode === 'spy2' && rooms[roomId].kickedPlayers) {
                remainingPlayersCount -= rooms[roomId].kickedPlayers.length;
            }
...
io.on('connection', (socket) => {
    ...
    socket.on('kickPlayer', (data) => { let tid = data.targetId; let roomId = socket.roomId || data.fallbackRoomId; if(roomId && rooms[roomId] && rooms[roomId].players[tid]) { delete rooms[roomId].players[tid]; emitUpdatedPlayers(roomId); io.to(tid).emit('youAreKickedPermanently'); } });
    ...
    socket.on('startVotingPhase', (fallbackRoomId) => { 
        ...
        if(roomId && rooms[roomId]) { 
            ...
            let totalPlayers = Object.keys(rooms[roomId].players).length;
            if (rooms[roomId].gameMode === 'spy2') totalPlayers -= (rooms[roomId].kickedPlayers ? rooms[roomId].kickedPlayers.length : 0);
