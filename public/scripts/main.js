...
// 🔥 تحديد وضع الكمبيوتر أو الموبايل أوتوماتيك وعرض الماوس الصح
function detectDevice() {
    if (window.innerWidth > 768) {
        document.body.className = 'pc-mode';
        document.getElementById('pcViewBtn').classList.add('active-view');
        document.getElementById('mobileViewBtn').classList.remove('active-view');
    } else {
        document.body.className = 'mobile-mode';
        document.getElementById('mobileViewBtn').classList.add('active-view');
        document.getElementById('pcViewBtn').classList.remove('active-view');
    }
}
detectDevice();
...
function showScreen(screenName) {
    ...
    // إخفاء الشاشة الرئيسية لو اللعبة بدأت
...
// 🔥 زراير الانضمام وإصلاح مشكلة الدخول التلقائي (استخدام sessionStorage للروم)
let isHost = false; 
...
document.getElementById('confirmHostPasswordBtn')?.addEventListener('click', () => {
    ...
    // 🔥 الحفظ هنا مؤقت في الجلسة دي بس عشان ميدخلوش غصب المرة الجاية
...
function renderCategories() {
    ...
    cats.forEach(cat => { let d=document.createElement('div'); d.className='category-card'; d.innerText=cat; d.onclick=()=>{playSound('click'); socket.emit('startGameWithCategory', cat, sessionStorage.getItem('hostRoomId'));}; grid.appendChild(d); });
}
let myRoleData = null;
