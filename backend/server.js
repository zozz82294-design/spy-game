...
io.on('connection', (socket) => {
    ...
    socket.on('destroyRoom', () => {
        ...
        if (roomId && rooms[roomId] && rooms[roomId].players[socket.playerId] && rooms[roomId].players[socket.playerId].isHost) {
            io.to(roomId).emit('hostLeftRoom', rooms[roomId].players[socket.playerId].name);
            cleanupRoom(roomId);
        }
    });
    ...
    function startNextRebusRound(roomId) {
        ...
        io.to(roomId).emit('rebusRoundStarted', { round: rooms[roomId].currentRound, clue: puz.clue, category: puz.category });
