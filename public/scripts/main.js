...
function showSuggestions(type) {
    ...
    if (type === 'hints') {
        ...
        pool = [...catHints];
        if (specificWordHints[word]) { pool = [...specificWordHints[word], ...pool]; }
    } else {
...
socket.on('connect', () => { 
    ...
    const hostRoomId = sessionStorage.getItem('hostRoomId'); const guestName = sessionStorage.getItem('guestName'); 
    if (hostRoomId) { 
...
if(joinRoomBtn) joinRoomBtn.addEventListener('click', () => { 
    ...
    showScreen('waitingScreen'); audioWaiting.play().catch(e => console.log(e)); 
});

if (destroyRoomBtn) { destroyRoomBtn.addEventListener('click', () => { if (confirm('هل أنت متأكد من إغلاق الغرفة وطرد جميع اللاعبين للعودة للقائمة الرئيسية؟')) { socket.emit('leaveRoom'); sessionStorage.removeItem('hostRoomId'); sessionStorage.clear(); window.location.href = window.location.pathname; } }); }
...
// 🔥 التحديثات والترتيب الحي بالنقاط
socket.on('updatePlayers', (playersArray) => {
    ...
    if (playersListDiv) { 
        ...
        playersArray.forEach(p => { 
            ...
            playersHTML += `<div class="player-item"><div class="player-avatar">👤</div><div class="player-info" style="width:100%;"><div class="player-name">${p.name}</div><div class="player-status" style="display:flex; align-items:center;">${hostBadge}${youBadge}${scoreBadge}</div></div></div>`; 
        }); 
        playersListDiv.innerHTML = playersHTML; 
    }

    if (isHost) { if (playersArray.length >= 2) { if(startGameBtn) startGameBtn.classList.add('hidden'); if(actualStartBtn) actualStartBtn.classList.remove('hidden'); actualStartBtn.innerText = currentGameMode === 'spy' ? "اختيار التصنيف 🚀" : "بدء الجولة الأولى 🚀";} else { if(startGameBtn) { startGameBtn.innerText = "في انتظار باقي اللاعبين... ⏳"; startGameBtn.classList.remove('hidden'); } if(actualStartBtn) actualStartBtn.classList.add('hidden'); } } else { if(startGameBtn) { startGameBtn.innerText = "في انتظار الهوست ⏳"; startGameBtn.classList.remove('hidden'); } if(actualStartBtn) actualStartBtn.classList.add('hidden'); }
    
    if (isHost && modalPlayersList) { 
        let modalHTML = ''; 
        playersArray.forEach(p => { 
            const crown = p.isHost ? '<span style="color:#ff0055;">👑</span>' : ''; const isMe = p.id === myPlayerId; 
            const actionButtons = p.isHost ? `<button class="btn-sidebar edit" style="width:auto; padding:5px 10px;" onclick="editPlayerName('${p.id}')">✏️</button>` : `<button class="btn-sidebar edit" style="width:auto; padding:5px 10px;" onclick="editPlayerName('${p.id}')">✏️</button><button class="btn-sidebar btn-danger" style="width:auto; padding:5px 10px; margin-right:5px;" onclick="kickPlayer('${p.id}')">❌</button>`; 
            modalHTML += `<div class="modal-player-item"><div class="player-name-wrapper"><span class="player-name-text">${p.name}</span>${crown}${isMe ? '<span style="color:#00ff88; font-size:0.8rem;">(أنت)</span>' : ''}</div><div class="modal-player-actions">${actionButtons}</div></div>`; 
        }); 
...
if(actualStartBtn) actualStartBtn.addEventListener('click', (e) => { 
    ...
    if(currentGameMode === 'spy') socket.emit('goToModeSelection', sessionStorage.getItem('hostRoomId')); 
    else socket.emit('startRebusGame', sessionStorage.getItem('hostRoomId'));
    setTimeout(() => e.target.disabled = false, 1000); 
});

socket.on('showModeSelection', () => { showScreen('modeSelectionScreen'); chosenCategory = null; isWheelSpinning = false; confirmStartGameBtn.classList.add('hidden'); renderCategories(); });
...
// 🔥 تعديل رسالة الشات لتصبح هادية ومن غير نقط
